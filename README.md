...
`pGen workers` はOLGA pGen計算の並列数です。通常作業をしながら使う場合は `4` を推奨します。PCを解析専用にできる場合だけ、必要に応じて増やします。
...
```powershell
.\.venv\Scripts\python.exe .\airr_pgen_shm_plot_beta1.py --input sample.igblast.airr.tsv --outdir result --sample SAMPLE --pgen-workers 4
```
