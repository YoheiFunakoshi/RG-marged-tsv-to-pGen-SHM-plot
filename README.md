...
現在の解析フローは、merged sequence -> IgBLAST AIRR TSV -> 採用フィルタ -> row-level Excel/TSV -> pGen/SHM/pGen-SHM図表です。

このREADMEは、GitHub上で全体像を確認するための最小限の説明です。再現に必要な詳細仕様、採用フィルタ、pGen/SHM計算、議論点は下記のWord仕様書を参照してください。

## 出力

入力TSVを1つ選んで実行すると、出力フォルダに以下を作成します。現在の主解析は、フィルタ後AIRR各行を保持する row-level 方式です。
...
Beta1互換出力は、旧解析・前任者法との比較のために残している補助出力です。現在の主解析は row-level Excel/TSV と row-level pGen-SHM plot です。

## Word仕様書

処理単位ごとの詳細仕様は、以下のWord文書に分けています。このリポジトリはpublicなので、URLを共有された人もダウンロードできます。GitHub上では `.docx` の本文が直接プレビューされないことがあるため、以下のリンクからWordファイルをダウンロードし、Microsoft Wordなどで開いてください。

- [pGen-SHM仕様書をダウンロード](https://raw.githubusercontent.com/YoheiFunakoshi/RG-marged-tsv-to-pGen-SHM-plot/main/docs/pgen_shm_spec_20260607.docx): 全体フロー、採用フィルタ、row-level Excel/TSV、pGen-SHM plot、議論点。
- [pGen仕様書をダウンロード](https://raw.githubusercontent.com/YoheiFunakoshi/RG-marged-tsv-to-pGen-SHM-plot/main/docs/pgen_spec_20260607.docx): OLGA human_B_heavyによるpGen計算、cache、pGen=0、pGen bins。
- [SHM仕様書をダウンロード](https://raw.githubusercontent.com/YoheiFunakoshi/RG-marged-tsv-to-pGen-SHM-plot/main/docs/shm_spec_20260607.docx): v_identityからのSHM計算、V alignment length、SHM histogram。

同じ内容の日本語ファイル名版もリポジトリ直下に残していますが、ブラウザ互換性のため、共有時は上記の英数字ファイル名リンクを使ってください。

## 最小限の解析概要

詳細な再現手順はWord仕様書を正とします。READMEでは、GUIの動作を把握するための要点だけを示します。

- 対象は基本的に IGH です。
- `locus` 列があり、値が `IGH` 以外の行は除外します。
- `locus` が空欄の行は保持し、QC summaryに件数を記録します。
- `productive == T` を主条件として採用します。これはIgBLAST/AIRRの総合判定で、V-J in-frame、stop codonなし等を反映します。
- `vj_in_frame` 列がある場合は、productive判定の内訳確認として `T` を採用します。列がない場合は適用しません。
- `stop_codon` 列がある場合は、productive判定の内訳確認として `F` を採用します。列がない場合は適用しません。
- `junction` は A/C/G/T のみを採用します。
- `junction_aa` は空欄、`*`、`X` を除外します。
- 現行フローでは `junction_aa` の先頭C・末尾W/Fを必須条件にはしていません。マージ済み配列由来のAIRRでは非canonicalな `junction_aa` も観測されるため、探索段階では中間表に残して確認します。
...
- `beta1_unique_junction` 出力は補助出力です。旧Beta1解析・前任者法との比較用であり、現在の主解析ではありません。1点を unique `junction` とし、SHMは同一junction内のmedianを使います。
- `pgen_shm_roi_summary.tsv` は低SHM・高pGen領域などの固定ROIについて、row-level方式とBeta1互換方式の件数を並べて出力します。
- `pGen=0` はOLGA `human_B_heavy` モデル上で生成確率0またはモデル外として扱われた配列です。生成されやすい方向ではなく、`log10(0)` が定義できないため、`<sample>_pgen_shm_rows.xlsx` と `<sample>_pgen_shm_points.tsv` には残し、scatter/KDE plotからは除外します。
...
- Beta1互換出力は、旧解析・前任者法との比較用に unique `junction` ごとのSHM medianを1点として保存する補助出力です。現在の主解析はrow-level方式です。
